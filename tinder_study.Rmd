...
```{r}
messages = bentinder %>% dplyr::select(date,messages_sent,messages_received) %>% mutate(message_differential = messages_received - messages_sent)

bentinder = bentinder %>% mutate(messages = messages_sent + messages_received) %>% dplyr::select(-c(messages_sent,messages_received))
...
rates = bentinder %>% dplyr::select(date,swipe_right_rate,match_rate) 
...
bentinder = bentinder %>% dplyr::select(-c(likes,passes,swipe_right_rate,match_rate))
...
tidy_messages = messages %>% dplyr::select(-message_differential) %>% gather(key = 'key',value = 'value',-date)
...
```{r}
nodates = dplyr::select(bentinder,-date)
